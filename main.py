...
def _load_icon(name):
    ...
    try:
        import cairosvg
        png_bytes = cairosvg.svg2png(bytestring=svg_data.encode(), output_width=target_size, output_height=target_size)
...
class HyprStatsWindow(Gtk.Window):
    def __init__(self):
        ...
        for i, (key, title) in enumerate(rows):
            ...
            # Sloupec 4: Teplota (bez samostatného marginu, bere spacing z gridu)
    ...
    def update_stats(self):
        ...
        self.cells["ram"]["detail"].set_text(f"{used_gb:.2f} / {total_gb:.2f} GB")
        self.cells["ram"]["temp"].set_text("")

        # CPU
        cpu_percent = psutil.cpu_percent()
        cpu_freq = psutil.cpu_freq()
        freq_text = f"{cpu_freq.current:.0f} / {cpu_freq.max:.0f} MHz" if cpu_freq else "N/A"
        ...
        self.cells["disk"]["detail"].set_text(f"{disk_used:.1f} / {disk_total:.1f} GB")
    ...
    def get_gpu_info(self):
        try:
            ...
            if result.returncode == 0:
                ...
                if len(parts) >= 3:
                    return f"{parts[0]}%", f"{parts[1]} / {parts[2]} MB"
        except Exception:
            pass
        ...
        for card_path in glob.glob("/sys/class/drm/card*/device"):
            try:
                ...
                return f"{gpu_percent}%", f"{vram_used:.0f} / {vram_total:.0f} MB"
            except Exception:
                continue
